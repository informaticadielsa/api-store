    ...
    IntegracionInfoTransferProyectos: async(req, res, next) => {
        try {
            ...
            for (let index = 0; index < resultJson.proyectos.length; index++) {
                ...
                integracionEmail('Usuario :'+element.codigoCliente +'contador:'+resultJson.proyectos.length+ JSON.stringify(evaluacion))
                if(evaluacion) {
    ...
    IntegracionInfoTransferProductosSetPrecioBaseFromListasPrecios: async(req, res, next) =>{
        try{
          ...
          var USDValor = constTipoCambio.cmm_valor
            ...
            let cadenaTipo = 'Integracion Transfer Productos set Prices Nuevos: correctamente. Tipo cambio:'+String(USDValor)
