...
export default{
    ...
    createProductosKitComponente: async(req, res, next) =>{
        try{
            const creadoConExito = await models.ProductosKitComponentes.create(req.body);
            res.status(200).send({
                message: 'Componente de kit creado'
            })

        }catch(e){
            res.status(500).send({
                message: 'Error al crear el componente de kit de producto',
                e
            });
            next(e);
        }
    ...
    //     }
    // },
    deleteProductosKitComponente: async(req, res, next) =>{
        try{

                //Busca si sku del producto existe para poder actualizarlo a que no ex kit
                const constProductosKitComponentes = await models.ProductosKitComponentes.findOne(
                {
                    where: {
                        prodkitcomp_productos_kits_componentes_id: req.body.prodkitcomp_productos_kits_componentes_id
                    },
                    attributes: {exclude: ['createdAt', 'updatedAt']}   
                });

                if(constProductosKitComponentes)
                {
                    await constProductosKitComponentes.destroy();
                }


            res.status(200).send({
                message: 'Componente de Kit eliminado exitosamente'
            })
        }catch(e){
            res.status(500).send({
                message: 'Error al eliminar el Componente de Kit',
                e
            });
            next(e);
        }
